...
布线预算受限时，请求可携带可选的 `max_edges`：只在**段数不超过该预算**的
子网中比较成本，成功响应额外回显实际段数 `edge_count`；若所有端点连通但
没有任何满足预算的子网，返回稳定的 422 并指出最少可行段数。未提供该字段
的请求与既有结果逐项兼容。

- API：Python 3 标准库实现，零第三方依赖。
- 求解内核：C11 实现的终端子集动态规划（Dreyfus–Wagner）、节点汇聚合并
  （并查集）与多源最短路闭包（多源 Dijkstra）；预算约束下段数直接进入
  终端子集状态与分层最短路闭包；不枚举边集、不调用通用优化器。
...
- `verify` 等 `api` 健康后运行一次：求解器单元测试 → 构建产物检查 →
  HTTP 冒烟（含并列裁决、无解边界、受限最优、预算不足与旧请求兼容），
  随后以退出码报告结果（0 成功）。
...
可选字段 `max_edges`（正整数，≤ 220）：布线预算，限制所选子网的段数。
提供时只有不超过该段数的子网参与成本比较；省略时求解不受段数约束，
响应与既有行为逐项一致。
...
- `edge_count`：**仅当请求提供 `max_edges` 时**额外出现，回显所选子网
  的实际段数（恒等于 `edge_set` 的长度且不超过预算）。
...
| 400 | `INVALID_MAX_EDGES` / `NON_POSITIVE_MAX_EDGES` / `MAX_EDGES_OUT_OF_RANGE` | `max_edges` 类型或取值问题（指针 `/max_edges`） |
| 422 | `DANGLING_ENDPOINT` | 端点无任何关联边（悬空） |
| 422 | `ENDPOINTS_UNCONNECTED` | 端点位于不同连通分量（无法连通），附 `components` |
| 422 | `EDGE_BUDGET_EXCEEDED` | 端点连通但没有任何子网满足 `max_edges`，附 `min_edges`（最少可行段数），不返回部分边集 |
...
3. **多源最短路闭包**：每个子集以全部顶点的合并标签为种子，跑一次
   多源 Dijkstra 完成闭包松弛。
4. **段数预算**：提供 `max_edges` 时，段数直接进入 DP 状态——
   `cur[v][e]` 为至多 `e` 段连通子集且触及 `v` 的最小代价，闭包在
   （顶点，已用段数）分层图上跑多源 Dijkstra（每条弧消耗一段）；合并
   只对 Pareto 前沿（at-most 曲线的严格降本记录点）做卷积。预算
   ≥ n−1 时约束不可能收紧，自动走无约束路径。受限最优直接由该 DP
   得出，**不是**先求无约束解再截断。
5. **规范见证**：仅在 DP 状态中保留单一（代价，见证）对不能保证全局
   字典序最优。因此先求出最优成本 `B`，再按边标识 ASCII 升序做贪心前缀
   扫描——对每条边询问“是否存在包含已选前缀与该边、成本恰为 `B` 且
   不超预算的连通树”，该 oracle 用并查集收缩必选边（其段数计入剩余
   预算）后在收缩多重图上再做一次（受限）Steiner DP。这是多项式次数的
   DP 调用（≤ 边数 + 1），不枚举边集。
6. **预算不足判定**：若受限 DP 不可行，以单位权重再跑一次终端子集 DP
   （最小化树边数），得到最少可行段数 `min_edges` 并区分于端点不连通。

`scripts/cross_validate*.py` 对 5000+ 随机小图与暴力枚举逐项核对
（成本、无解、字典序见证、平行边、并列、随机段数预算下的受限最优与
最少可行段数）。
